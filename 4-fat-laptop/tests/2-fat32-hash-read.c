...
void notmain() {
  ...
  for (int i = 0; i < files.ndirents; i++) {
    ...
    read_and_hash(&fs, &root, "HELLO-F.BIN", 0x70a2773a);
    ...
    // read_and_hash(&fs, &root, "BOO~125.BIN", 0xed7e8fc7);
    // read_and_hash(&fs, &root, "BOO~126.BIN", 0xed7e8fc7);
    // read_and_hash(&fs, &root, "BOOT~117.BIN", 0xed7e8fc7);
